    ...
    def construct(self):
        ...
        for i in range(n):
            ...
            if l[i]==key:
                ...
                # t4=Text("Element found at index "+str(i+1), t2c={str(i+1): BLUE}).scale(0.6).next_to(t3, DOWN)
            ...
            # self.play(tab.get_rows()[0][i].animate.move_to(cell), run_time=1)


        if pos==-1:
            t3=Text("Key not found", color=BLUE).scale(0.6).move_to(t2.get_center())
        else:
            # t4=Text("Index", color=GREY_B).scale(0.6).next_to(tab, DOWN+LEFT).scale(0.6)
            # self.play(FadeIn(t4))
